...
class DQNAgent:
    ...
    def __init__(self, model, target_model, state, img_send, pub, sub, num_actions, buffer_size=100, learning_rate=.001, epsilon=.1,
                 epsilon_decay=0.995, min_epsilon=.01, gamma=.9, batch_size=4, target_update_iter=10, train_nums=20, 
                 start_learning=5):
    ...
    def train(self):
        ...
        # 3. get the e-greedy action (Max Q or random action)
            ...
            # self.pub.act = action
            # self.pub.pending = 3
            # print("Selected action: ", self.pub.act) 
            print("action", action)     
        ...
        # 6. take the action in the env to return s', r, done
            ...
            if done:
                ...
                while (not rospy.is_shutdown()):
                    self.pub_to_action.pub_reset_00.publish(data=True)
